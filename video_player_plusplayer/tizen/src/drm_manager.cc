...
int DrmManager::OnChallengeData(void *session_id, int msgType, void *msg,
  ...
  LOG_INFO("[DrmManager] challenge data length: %d", msgLen);
  ...
  if (!drm_manager->license_url_.empty()) {
    ...
    DRM_RESULT drm_result = CBmsDrmLicenseHelper::DoTransaction_TZ(
        ...
        &drm_manager->pb_response_len_,
        (CBmsDrmLicenseHelper::EDrmType)drm_manager->drm_type_, nullptr,
        nullptr);
    ...
    intptr_t ret = drm_manager->get_challenge_cb_(
        reinterpret_cast<uint8_t *>(&challengeData[0]),
        static_cast<size_t>(challengeData.length()), drm_manager->player_id_);
    if (ret == 0) {
  ...
  drm_manager->license_param_.param3 =
